...
import { deletePros } from "./actions";
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-lg font-medium">PROs</h1>
        <Link
          href="/pros/new"
      ...
      {error && <ErrorBanner message={`Couldn't load pros: ${error.message}`} />}

      {!error && (
        <BulkList
          rows={(data ?? []).map((row) => ({ id: row.id, primary: row.name }))}
          detailHrefBase="/pros"
          onBulkDelete={deletePros}
